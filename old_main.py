...
def run_entire_pipeline(folder,
                        multi_modal_weights,
                        patch_sizes,
                        descriptorsgaussian_mixture_keep_portion,
                        pca_comp,
                        folds,
                        number_of_gaussians,
                        samples_per_calss,
    ...
    folder_met_data = folder.replace(
        os.path.join(folder.split("/")[-2], folder.split("/")[-1]), "pollution_data"
    )
    ...
    if (samples_per_class <= 0):  # to get the number of samples that has the folder with less samples
        samples_per_class = min(
            np.asarray(
                [
                    len(os.listdir(os.path.join(folder, f)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_folder",
        help="specify the folder with the image_data (deafault = "
    ...
    parser.add_argument(
        "--multi_modal_weights",
        help="weights for multi modal classification (image feature weights, metheorological feature weights, default = "
        + str(default_multi_modal_weights)
        + ")",
        default=default_multi_modal_weights,
        type=float,
        nargs=2,
        action="store",
    )
    parser.add_argument(
        "--gmm_keep_portion",
        help="portion of the data used to compute the Gaussian Mixture Model (default = 0.5)",
        default=0.5,
        type=float,
    )
    parser.add_argument(
        "--pca_comp",
        help="number of PCA components (default = " + str(pca_comp) + ")",
        default=pca_comp,
        type=int,
    )
    parser.add_argument(
        "-F",
        "--folds",
        help="number of folds performed in cross validation (default = "
        + str(folds)
        + ")",
        default=folds,
        type=int,
    ...
    parser.add_argument(
        "--descriptors",
        help="descriptors used for as features (can be: 'sift', 'centrist', 'color' defaults = "
        + str(default_descriptors)
        + ")",
        type=str,
    ...
    parser.add_argument(
        "--patch_sizes",
        help="patch sizes used for the descriptors (can be: 12, 16, 20, defaults = "
        + str(default_patch_sizes)
        + ")",
        type=int,
    ...
    parser.add_argument(
        "-GMM",
        "--number_of_gaussians",
        help="number of gaussians used per gaussian mixture (default = "
        + str(number_of_gaussians)
        + ")",
        default=number_of_gaussians,
        type=int,
        nargs=2,
        action="store",
    )
    parser.add_argument(
        "--normalizaton_type",
        help="normalizaton used for the data (can be: 'norm', 'scale', if something else no normalization will be applied, default is "
        + str(normalizaton_type)
        + ")",
        default=normalizaton_type,
        type=str,
    )
    parser.add_argument(
        "-R",
        "--regresor",
        help="regresor used (can be: 'SVM'(support vector machine), 'ELN' (elasticnet), 'BAY' (bayesian ridge), 'LIN' (linear regression), 'LASSLAR' (lasso least angle regression), 'LASS'  (lasso regression), 'THE' (theil sen regression), 'ARDR' (bayesian ARD regression), deafult is "
        + default_regresor[0]
        + ")",
    ...
    parser.add_argument(
        "-N",
        "--samples_per_calss",
        help="number of samples per class (AQI index) used (default = "
        + str(default_samples_per_class)
        + ")",
    ...
    parser.add_argument(
        "--use_saved_gmm",
        help="use an already saved GMM for the features (if posible) ()",
        default=use_latest_gmm,
        type=int,
    )
    parser.add_argument(
        "--save_to_drive",
        help="save results and GMM to google drive (deafult = "
        + str(save_to_drive_boolean)
        + ")",
        default=save_to_drive_boolean,
        type=int,
    )
    parser.add_argument(
        "--save_results",
        help="save results (txt and histograms if generated, default = "
        + str(save_results)
        + ")",
    ...
    parser.add_argument(
        "--save_gmm",
        help="save gaussian mixture models generated (default = " + str(save_gmm) + ")",
        default=save_gmm,
        type=int,
    )
    parser.add_argument(
        "--print_hist", help="Show histograms (deafult = 0)", default=0, type=int
    )
    parser.add_argument(
        "--save_hist_folder",
    ...
    parser.add_argument(
        "--not_all_data", help="to use just some classes of teh data", type=bool
    )
    parser.add_argument(
        "--run_number",
        help="number of the run if it belongs to a multiple run, this is done to create a result folder for each run",
        type=int,
    )
    ...
    run_entire_pipeline(multi_modal_weights)
