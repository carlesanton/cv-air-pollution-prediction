...
def get_features_from_sample_image(eng: matlab.engine, image_path, folder_to_save):
    ...
    if image_name.endswith(".jpg"):
        ...
        raw_desc_feat = eng.first_test(
            folder_path, image_name, data_path, [12, 16, 20], 8, 300
        )
...
def new_image_fv(visual_features, met_data, gaussian_models, multi_modal_weights):
    ...
    ) * u.fisher_vector(np.asarray(poll_data_from_sample), gaussian_models[count])
...
def load_gmm(gaussian_mixture_path):
    ...
    number_of_gaussians = int(
        gaussian_mixture_path.split("/")[-1].split("_")[2].replace("N", "")
    )
...
def fit_new_patch(
    ...
    image_feature = new_image_fv(
        visual_features, met_data, gaussian_mixture_models, multimodal_weigths
    )
...
def plot_histograms(histograms, boolean, folder_to_save, bins=50):
    ...
    for h in histograms:
        ...
        plt.plot(
            histogram[1][:-1], histogram[0], label="Train samples", color="darkgreen"
        )
        x_step = 1
...
[
    linear_models_path.append(
        models_folder + "/" + parameters_for_saving[r] + "_" + regresors[r] + ".sav"
    )
    for r in range(len(regresors))
]


gaussian_models_paths = []
gaussian_models_paths.append(
    "/Volumes/Data_HD/Users/Carles/TFG/codes/try_github/TFG_database/gmm/65_gmm_N200_598997s_k1_centrist12_else.npz"
)
gaussian_models_paths.append(
    "/Volumes/Data_HD/Users/Carles/TFG/codes/try_github/TFG_database/gmm/66_gmm_N10_3908s_k1_weather_else.npz"
)
...
if (
    samples_per_class <= 0
):  # to get the number of samples that has the folder with less samples
    samples_per_class = min(
        np.asarray(
            [
                len(os.listdir(os.path.join(default_image_folder, f)))
                for f in os.listdir(default_image_folder)
                if os.path.isdir(os.path.join(default_image_folder, f))
            ]
        )
    )
clases_to_get = [
    int(i) for i in os.listdir(default_image_folder) if i != ".DS_Store" and i != "420"
]  # [42,129]
clases_to_get = [34, 51, 55]
(sample_names, sample_classes, sample_pollution_paths, sample_features) = u.get_samples(
    clases_to_get,
    samples_per_class,
...
# get met data
met_test_samples = np.vstack(
    [
        u.retrieve_weather_data_from_sample(sample_pollution_paths[i], sample_names[i])
...
for i in range(len(sample_names)):
    patch_path = os.path.join(
        sample_pollution_paths[i].replace("pollution_data", "image_data"),
        sample_names[i],
    )
    u.delete_last_lines()
    print(
        ...
        + str(i)
        + "/"
        + str(len(sample_names))
        + ")"
    )
    (predicted_pollution, name_of_pollutant) = fit_new_patch(
        patch_path,
        linear_models,
        gaussian_mixture_models,
        multimodal_weigth,
...
for p in range(len(name_of_pollutant)):
    ...
    pollutant_predicted_samples = poll_predicted_array[:, p][
        samples_with_that_pollution_data
    ]
...
plot_histograms(
    results_histograms,
    [1, 1],
    "/Volumes/Data_HD/Users/Carles/TFG/codes/histograms/30877",
    bins=n_bins,
)
