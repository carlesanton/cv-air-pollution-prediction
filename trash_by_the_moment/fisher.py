...
def dictionary(descriptors, N, responses):
    ...
    # em.train(descriptors,cv2.ml.ROW_SAMPLE,responses)
    print(len(em.getMeans()))
    print(len(em.getCovs()))
    print(len(em.getWeights()))
    print(em.getWeights())
    return (
        np.float32(em.getMeans()),
        np.float32(em.getCovs()),
        np.float32(em.getWeights())[0],
    )
...
def fisher_vector_means(s0, s1, s2, means, sigma, w, T):
    return np.float32(
        [
            (s1[k] - means[k] * s0[k]) / (np.sqrt(w[k] * sigma[k]))
...
def fisher_vector_sigma(s0, s1, s2, means, sigma, w, T):
    return np.float32(
        [
            (s2[k] - 2 * means[k] * s1[k] + (means[k] * means[k] - sigma[k]) * s0[k])
...
def fisher_vector(samples, means, covs, w):
    ...
    print(c)
    fv = np.concatenate(
        [np.concatenate(b), np.concatenate(c)]
...
def generate_gmm_v2(words, N, responses):
    # words = np.concatenate([folder_descriptors(folder) for folder in glob.glob(input_folder + '/*')])
    print("Training GMM of size", N)
    means, covs, weights = dictionary(words, N, responses)
    print(means.shape)
    # [print(m) for m in means]
    print(covs.shape)
    # [print(m) for m in covs]
    print(weights.shape)
...
if __name__ == "__main__":
    ...
    gmm = (
        load_gmm(working_folder)
        if args.loadgmm
        else generate_gmm(working_folder, args.number)
    )
