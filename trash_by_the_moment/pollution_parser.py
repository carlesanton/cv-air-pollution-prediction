...
def parse_pollution_csv(cursor: sqlite3.Cursor, path, table_name):
    ...
    for row in reader:
        ...
        if reader.line_num != 1:
            ...
            lon = row[6]
            cursor.execute(
                inster_string, (count, site, date_time, sample, unit, lat, lon)
            )
            if count % 10000 == 0:
...
def parse_aqi_csv(cursor: sqlite3.Cursor, path):
    ...
    for row in reader:
        ...
        if reader.line_num != 1:
            ...
            def_param = row[7]
            cursor.execute(
                "insert into poll_aqi values (?,?,?,?,?,?)",
                (count, site, date_time, sample, category, def_param),
            )
            if count % 10000 == 0:
                print(
                    "Inserted sample: "
...
def parse_gas_folder(cursor: sqlite3.Cursor, path):
    switcher = {44201: "gas_ozone", 42401: "gas_so2", 42101: "gas_co", 42602: "gas_no2"}
...
def get_sites_for_parameter(cursor: sqlite3.Cursor, k, switcher, avoid):
    if np.sum([b + 1 for b in avoid if b == k], dtype=bool):  # si l'hem d'evitar
        ...
        print(
            "Getting sites from table: " + switcher.get(k, "Invalid parameter code"),
            end="\r",
        )
        return get_fields_from_table(
            cursor,
            switcher.get(k, "Invalid parameter code"),
            "distinct site, lat, lon",
            " ",
        )


def create_camera_sites_relation_table(
    cursor: sqlite3.Cursor, cam_cursor: sqlite3.Cursor
):
    radi_threshold = 20000  # mig perimetre de la terra
    cursor.execute(
        """CREATE TABLE IF NOT EXISTS camera_sites (camera_id INTEGER PRIMARY KEY,
    ...
    cams = get_fields_from_table(
        cam_cursor, "cameras", "id,lat,long", "where country = 'USA'"
    )  #
    radi_original = 100
    Numero_param = 10
    valors = [["??" for j in range(2)] for i in range(Numero_param)]
    query_taules = [
        get_sites_for_parameter(cursor, n, param_switcher, [4, 5])
        for n in range(Numero_param)
    ]  # diu els sites on hi ha una mostra d'aquell contaminant
    for c in cams:  # per cada camara a USA
        ...
        while np.sum(
            need, dtype=bool
        ):  # mentre no haguem trobat un site per cada parametre
            bbox = get_bounding_box(
                c[1], c[2], radi
            )  # potser va fora del while pero no te pinta
            for n in [
                x for x in range(0, len(need)) if need[x]
            ]:  # per cada paramametre que calgui comprovar
                print(
                    "Getting cam "
                    + str(c[0])
                    + " parameter number "
                ...
                sites = [
                    q for q in query_taules[n] if in_bounding_box(bbox, q[1], q[2])
                ]  # per cada possible site del contaminant mira si esta a la bbox i si ho esta el posa a la llista
                ...
                if len(sites) != 0:  # SI HEM TROBAT SITES EN AQUELL RADI
                    distances = [
                        lat_lon_2_dis(c[1], site[1], c[2], site[2]) for site in sites
                    ]  # calculem la distancia a cada un dells
                    sorted_indexes = np.argsort(
                        distances
                    )  # agafem els index de les distancies ordenades
                    sorted_sites = np.array(sites)[
                        sorted_indexes
                    ]  # ordenem els sites amb els index
                    valors[n][0] = str(sorted_sites[0][0])  # guardem el site que sigui
                    valors[n][1] = math.floor(
                        distances[sorted_indexes[0]]
                    )  # amb la seva corresponent distancia
                    need[n] = False
        ...
        print(
            "Camera: "
            + "{:>6}".format(final_values[0])
            + "".join(
                [
                    "|||"
                    + "{:11}".format(valors_1d[n])
                    + " "
                    + "{:>7}".format(valors_1d[n + 1])
                    for n in range(0, len(valors_1d), 2)
                ]
            )
            + "||| with maximum radi: "
            + str(radi)
...
def update_camera_sites_table(cursor: sqlite3.Cursor, cam_cursor: sqlite3.Cursor):
    ...
    valors = [["??" for j in range(2)] for i in range(Numero_param)]
    query_taules = [
        get_sites_for_parameter(cursor, n, param_switcher, [0, 1, 2, 3, 6, 7, 8, 9])
        for n in range(Numero_param)
    ]
    for c in cams:
        get_sites = []
        ...
        final_values = tuple(insert_values)
        cursor.execute(
            "update camera_sites set PM25 = ?, PM25_DISTANCE = ?, PM10 = ?, PM10_DISTANCE = ? WHERE camera_id= ?",
            (valors[4][0], valors[4][1], valors[5][0], valors[5][1], c[0]),
        )
        print(
            "Camera: "
            + "{:>6}".format(final_values[0])
...
def get_cameras_with_sites_closer_than(cursor: sqlite3.Cursor, distancia_maxima, field):
    ...
    condition = (
        ...
        + " AND PM10_DISTANCE < "
        + str(distancia_maxima)
        + " AND O2 != '??' AND SO2 NOT LIKE '??' AND CO !='??' AND NO2 !='??' AND WIND != '??' AND TEMP !='??' AND PRES !='??' AND RH !='??' AND PM25 !='??' AND PM10 !='??'"
    )
    cameras = get_fields_from_table(cursor, table_name, fields, condition)
    return cameras


def get_data_from_cameras_in_list(
    cam_ids_list, all_pollution_data, date_string, destiny_path
):
    ...
    save_pollution_data_to(
        cam_data, "2017-01-01 14:00", destiny_path + "/pollution_data"
    )
...
def filter_cam_data(all_data, par_ids, date_string):
    ...
    for i in range(len(all_data)):
        ...
        filtered_data = [
            s[2] for s in all_data[i] if check_cam_date(s, par_ids[i], date_string)
        ]
...
def get_all_pollution_data(cursor: sqlite3.Cursor):
    ...
    for i in range(0, 10):
        ...
        parameter_data = get_fields_from_table(
            cursor,
            param_switcher.get(n, "Invalid parameter name"),
            "site, data, sample",
            "",
        )
...
def get_all_pollution_data_for_cam(cursor: sqlite3.Cursor, cam_id):
    all_pollution_data = []
    s = time.time()
    cam_parameters = get_fields_from_table(
        cursor,
        "camera_sites",
        "O2, SO2, CO, NO2, PM25, PM10, WIND, TEMP, PRES, RH",
        str("where camera_id=" + str(cam_id)),
    )[0]
...
def get_cameras_data_from_date(super_data_array, cam_id, date):
    ...
    get_condition = (
        "where site = '" + camera_sites[0][0] + "' AND data = '" + date + "'"
    )
    print(get_condition)
...
def save_pollution_data_for_cam_images(
    cam_images_folder, all_pollution_data, cam_parameter_ids
):
    par_ids = [
        cam_parameter_ids[0][i] for i in range(len(cam_parameter_ids[0])) if i != 0
    ]
    ...
    for f in os.listdir(cam_images_folder):  # per cada carpeta on esta guardat tot
        if os.path.isdir(cam_images_folder + "/" + f):
            ...
            for image_name in image_days:
                ...
                final_txt_path = (
                    cam_images_folder + "/" + f + "/" + image_name + "_pollution.txt"
                )
                cc = 0
                for fff in filtered:
                    if len(fff) == 0:
...
def save_pollution_data_for_cam(db_cursor, cam_parameter_ids, save_to_folder):
    ...
    aqi_query_condition = (
        "where site LIKE '"
        + cam_parameter_ids[1].replace(cam_parameter_ids[1].split("-")[-1], "")
        + "%'"
    )  # que almenys estigui en el mateix condat
    all_aqi_site_data = get_fields_from_table(
    ...
    for aqi_index in all_aqi_site_data:  # per cada
        ...
        datetime_strings = [
            date_string + " " + "{:0>2}".format(i) + ":00" for i in range(24)
        ]
        for date in datetime_strings:
            ...
            txt_name = (
                date_split[0]
                + date_split[1]
                + date_split[2].split(" ")[0]
                + "_"
                + date_split[2].split(" ")[1].replace(":", "")
                + "00_pollution.txt"
            )
...
def get_day_par_info(all_data, par_ids, date_string, par_num):
    filtered_data = [
        s for s in all_data[par_num] if check_cam_date(s, par_ids[par_num], date_string)
    ]
