...
def put_images_to_aqi_folder(
    camera_id, aqi_data, cam_ids, path_prov, path_patches, path_end
):
    patches_to_do = mp.list_from_patches_txt(
        path_patches + "/" + "{:0>8}".format(str(camera_id)) + ".txt"
    )
    # print(patches_to_do)
    ...
    for f in os.listdir(path_prov):  # per cada foto a la carpeta
        if f.endswith(".jpg"):
            area_t = 40
            pix_t = 70
            if mp.check_night_image(
                path_prov + "/" + f, area_t, pix_t
            ):  # if  a night image
                continue
                ...
                aqi_index = [
                    r
                    for r in aqi_data
                    if [True for p in cam_ids if p == r[0]] and r[1] == date_to_get
                ]
                if (
                    len(aqi_index) == 0
                ):  # SI EL SITE QUE HA CALCULAT EL AQI NO ES CAP DELS QUE TE LA CAMARA PER ALGUN PARAMETRE AGAFEM UN DEL MATEIX CONDAT I JASTA
                ...
                try:
                    ...
                    mp.make_image_patches(
                        150,
                        path_prov,
                        f,
                        path_end + "/" + str(aqi_folder_index),
                        patches_to_do,
                    )
                    images_made += 1
                    # raise Exception
...
def add_thread(
    ...
    download_thread = DownloadThread(
        ...
        patches_info_path=patches_info_path,
        aqi_data=aqi_data,
        cam_ids=param_ids,
    )
