...
def patch_and_features_from_folder(camera_id, folder_location,final_data_path,patches_info_path,db_cursor,eng:matlab.engine):
    cameras_params=pp.get_fields_from_table(db_cursor,'camera_sites','camera_id,O2, SO2, CO, NO2, PM25, PM10','where camera_id='+str(camera_id))
    ...
    aqi_query_condition="where site LIKE '"+ camera_params[1].replace(camera_params[1].split('-')[-1],'') + "%'" #que almenys estigui en el mateix condat
    ...
    da.put_images_to_aqi_folder(camera_id,all_aqi_site_data,par_ids,folder_location,patches_info_path,final_data_path)
    print("Done")
...
final_data_path='/Volumes/Data_HD/Users/Carles/TFG/codes/try_github/TFG_database/image_data/'+'{:0>8}'.format(camera_id)
