...
title_switcher = {0: "Ozone", 1: "SO2", 2: "CO", 3: "NO2", 4: "PM2.5", 5: "PM10"}

dont_get_data_till_parametter = 5


def download_months_year_from_cameras(
    month_vector, year, cameras_to_get, src_path, db_cursor, SAVE_POLL_DATA: bool
):
    ...
    for c in cameras_to_get:
        for month in month_vector:
            ...
            aqi_query_condition = (
                "where site LIKE '" + c[1].replace(c[1].split("-")[-1], "") + "%'"
            ...
            final_data_path = (
                src_path + "/final_data/image_data" + "{:0>8}".format(c[0])
            )
            provisional_data_path = (
                src_path
                + "final_data/downloaded_images/"
                + "{:0>8}".format(c[0])
                + "_"
                + str(year)
            ...
            t = time.time()
            print(
                "Starting thread to download cam: "
                + "{:0>8}".format(c[0])
                + ", month: "
                + "{:0>2}".format(month)
                + ", year: "
...
def get_sample_names(
    ...
    for f in os.listdir(folder):
        if [
            True for c in clases_to_get if f == str(c)
        ]:  # si la carpeta es una classe que s'hagi d'agafar

            if os.path.isfile(
                folder + "/" + f + "/image_names.txt"
            ):  # check if there is a file with the folder names exists
                myfile = open(folder + "/" + f + "/image_names.txt", "r")
                file_lines = myfile.readlines()
                image_list = [
                    ff.replace("\n", "")
                    for ff in file_lines
                    if ff.replace("\n", "").endswith(".jpg")
                    and check_sample_has_all_pollution_data(
                        ff.replace("\n", ""),
                        f,
                        folder.replace("image_data", "pollution_data"),
                        class_type,
                    )
                ]
                if check_weather_data:
                    image_list = [
                        ...
                        for ff in file_lines
                        if ff.replace("\n", "").endswith(".jpg")
                        and check_sample_has_all_weather_data(
                            ff.replace("\n", ""),
                            f,
                            folder.replace("image_data", "pollution_data"),
                            class_type,
            ...
            if (
                len(image_list) < max_samples_per_class
            ):  # si hi ha menys samples dels que volem agafar els agafem igual pero
            ...
            for image in final_name_list:
                ...
                for descriptor in descriptors:
                    for patch in patch_sizes:
                        features_of_sample.append(
                            retrieve_feature_from_sample(
                                folder, image, final_path, descriptor, patch
                            )
                        )
            ...
            ):  # use as classes the 6 pollution samples for that image
                final_classes.extend(
                    [
                        retrieve_pollution_data_from_sample(
                            folder.replace("image_data", "pollution_data") + "/" + f,
                            final_name_list[i],
                        )
                        for i in range(samples_per_class)
                    ]
            ...
            print(
                "Got "
                + "{: >4}".format(samples_per_class)
                + " samples for AQI index "
                + "{: >3}".format(f)
                + " of cam "
                + folder.split("/")[-1]
            )
            [
                print(np.asarray(final_classes)[i, :])
                for i in range(np.asarray(final_classes).shape[0])
                if sum(np.asarray(final_classes)[i, :] > 150)
            ]
    return (final_samples, final_classes, final_paths, final_features)


def get_sample_names_meateorological(
    clases_to_get, samples_per_class, folder, camera_shuffle, cameras_to_get
):
    final_samples = []
    final_classes = []
    for aqi_index in clases_to_get:
        ...
        for cam_folder in cameras_to_get:  # per cada camara
            if os.path.isdir(os.path.join(folder, "{:0>8}".format(cam_folder))):
                for aqi_folder in os.listdir(
                    folder + "/" + "{:0>8}".format(cam_folder)
                ):  # per cada caprpeta de index
                    if os.path.isdir(
                        os.path.join(folder, "{:0>8}".format(cam_folder), aqi_folder)
                    ) and aqi_folder == str(aqi_index):
                        # print(aqi_folder + ': ' + str(aqi_index))
                        file_list = os.listdir(
                            folder
                            + "/"
                            + "{:0>8}".format(cam_folder)
                            + "/"
                            + aqi_folder
                        )
                        ...
                        [
                            ...
                            and check_sample_has_all_pollution_data(
                                ff,
                                aqi_folder,
                                os.path.join(folder, "{:0>8}".format(cam_folder)),
                            )
                        ]
        ...
        if camera_shuffle:
            shuffled_name_list = sample(
                text_list, len(text_list)
            )  # to get samples from every camera
...
def check_sample_has_all_pollution_data(
    patch_name, class_folder, folder_path, class_type
):
    ...
    if not os.path.isfile(
        os.path.join(
            folder_path.replace("pollution_data", "image_data"),
            class_folder,
            "features",
            str(patch_name.replace(".jpg", "") + "_12_centrist.mat"),
        )
    ):
        print(os.path.join(folder_path, class_folder, "features", patch_name))
        return False
    elif check_sample_has_all_weather_data(
        patch_name, class_folder, folder_path, class_type
    ):
        for f in lines:
            if not f:
...
def check_sample_has_all_weather_data(
    patch_name, class_folder, folder_path, class_type
):
    expected_name = list(patch_name.replace(str("_" + patch_name.split("_")[-1]), ""))
    expected_name[-4:] = list("0000")
...
def generate_gaussian_mixture_model(samples, number_of_gaussians):
    ...
    fv_gmm = sklearn.mixture.GaussianMixture(
...
def fisher_vector_encoding(
    pollution_data_paths,
    sample_name,
    sample_features,
    descriptors,
    patches,
    ...
    visual_fisher_vector = np.concatenate(
        [
            (multi_modal_weights[weigth_selector] / visual_fv_length) * f
            for f in sample_fisher_vector
        ]
    )
    weigth_selector += 1

    poll_data_from_sample = retrieve_weather_data_from_sample(
        pollution_data_paths, sample_name
    )
    poll_data_length = len(poll_data_from_sample)
    met_fisher_vector = (
        multi_modal_weights[weigth_selector] / poll_data_length
    ) * fisher_vector(np.asarray(poll_data_from_sample), gaussian_models[count])
...
def retrieve_feature_from_sample(
    folder_path, sample_name, pollution_data_folder, descriptor_name, patch_size
):
    ...
    expected_path = (
        folder_path + "/" + str(sample_aqi_value) + "/features/" + expected_name
    )
    try:
        array = scipy.io.loadmat(
            folder_path + "/" + str(sample_aqi_value) + "/features/" + expected_name
        )
        ...
        print(
            "Couldn't load feature "
            + expected_name
            + " from sample "
            + sample_name
            + " (AQI folder "
            + str(sample_aqi_value)
            + ")"
        )
...
def retrieve_feature_from_sample_(
    folder_path, sample_name, pollution_data_folder, descriptor_name, patch_size
):
    sample_aqi_value = pollution_data_folder.split("/")[-1]
    # returns array with dimensions (NxD) where N is the number of feature points extracted with that patch size and D the dimensionality of the descriptor
...
def count_samples_for_aqi_index(root_folder):
    ...
    [
        print("{:>3}".format(pair[0]) + ": " + "{:>5}".format(pair[1]))
        for pair in aqi_index_vector
        if pair[1] != 0
    ]
...
def save_pollution_data_for_cams(db_cursor, cameras_to_get, src_path):
    for c in cameras_to_get:
        ...
        pollution_parser.save_pollution_data_for_cam(
            db_cursor,
            c,
            src_path + "final_data/pollution_data" + "/" + "{:0>8}".format(c[0]),
        )
...
def get_samples(
    clases_to_get, samples_per_class, folder, class_type, descriptors, patch_sizes
):
    (
        sample_names,
        sample_classes,
        pollution_sample_path,
        sample_features,
    ) = get_sample_names(
        clases_to_get, samples_per_class, folder, class_type, descriptors, patch_sizes
    )
...
def get_samples_by_pollution(
    clases_to_get,
    samples_per_class,
    root_folder,
    cameras,
    class_type,
    just_data_with_image,
):
    ...
    if just_data_with_image:
        for c in cameras:  # for each camera we want to take the data
            for aqi_f in os.listdir(
                image_folder + "/" + "{:0>8}".format(c)
            ):  # for each aqi folder
                if os.path.isdir(image_folder + "/" + "{:0>8}".format(c) + "/" + aqi_f):
                    ...
                    prov_pollution_data = [
                        ...
                        for f in os.listdir(
                            image_folder + "/" + "{:0>8}".format(c) + "/" + aqi_f
                        )
                        if f.endswith("_pollution.txt") and f != ".DS_pollution.txt"
                    ]
                    prov_path_list = [
                        image_folder
                        + "/"
                        ...
                        + aqi_f
                        + "/"
                        + f.replace("_pollution.txt", "")
                        for f in os.listdir(
                            image_folder + "/" + "{:0>8}".format(c) + "/" + aqi_f
                        )
                        if f.endswith("_pollution.txt") and f != ".DS_pollution.txt"
                    ]
                    index_to_delete = np.arange(len(prov_path_list))[
                        np.sum(np.asarray(prov_pollution_data), 1) == 0.0
                    ]
    ...
    num_steps = 50
    steps = [
        (max_predict_range[i] - min_predict_range[i]) / num_steps
        for i in range(min_predict_range.shape[0])
    ]
    count = 0
    ...
    for step in steps:
        ...
        for poll_sample in pollution_data:
            sample_bin = int(
                num_steps
                * (
                    (poll_sample[count] - min_predict_range[count])
                    / (max_predict_range[count] - min_predict_range[count])
                    - 0.0001
                )
            )
    ...
    [
        print(str(np.amin(all_hists[:, a])) + " " + str(np.amax(all_hists[:, a])))
...
def cross_validation(
    sample_names,
    sample_classes,
    sample_pollution_paths,
    sample_features,
    image_folder,
    folds,
    multi_modal_weights,
    gaussian_mixture_keep_portion,
    number_of_gaussians,
    descriptors,
    patch_sizes,
    class_type,
    samples_per_aqi_index,
    regresors,
    save_results_boolean,
    histograms_boolean,
    save_to_google_drive_boolean,
    gmm_booleans,
    norm_type="norm",
    PCA=0,
):
    ...
    [
        print(np.asarray(sample_classes)[i, :])
        for i in range(np.asarray(sample_classes).shape[0])
        if sum(np.asarray(sample_classes)[i, :]) > 150
    ]
    for k in range(folds):
        ...
        s_tot = [
            compute_s_tot(np.asarray(test_labels)[:, i])
            for i in range(np.asarray(test_labels).shape[1])
        ]
        print(s_tot)
...
def fit_regressor_and_compute_metrics(
    regresor,
    final_train_samples,
    train_labels,
    final_test_samples,
    ...
    if class_type == "AQI":
        ...
        visualize_residual_plots(
            histograms_boolean,
            multi_regressor.estimators_,
            final_test_samples,
            test_labels,
            fold + 1,
        )
        ...
        # METRICS COMPUTATION
        root_mean_squared_errors = np.sqrt(
            metrics.mean_squared_error(
                (test_labels), (predicted_labels), multioutput="raw_values"
            )
        )
        predicted_mean = [
            compute_mean(predicted_labels[:, i])
            for i in range(len(root_mean_squared_errors))
        ]
        error_stdv = [
            compute_error_stdv(
                predicted_labels[:, i],
                np.asarray(test_labels)[:, i],
                root_mean_squared_errors[i],
            )
            for i in range(len(root_mean_squared_errors))
        ]
        r_squares = metrics.r2_score(
            np.asarray(test_labels), (predicted_labels), multioutput="raw_values"
        )
        s_ress = [
            compute_s_ress(predicted_labels[:, i], np.asarray(test_labels)[:, i])
            for i in range(len(root_mean_squared_errors))
        ]
        s_tot = [
            compute_s_tot(np.asarray(test_labels)[:, i])
        ...
        expected_r2 = 1.0 - (np.asarray(s_ress) / np.asarray(s_tot))
        stv = [
            compute_stdv(predicted_labels[:, i], predicted_mean[i])
            for i in range(len(root_mean_squared_errors))
        ]
        ...
        test_histograms = data_historgrams(np.asarray(test_labels))
        plot_histograms(
            train_histograms, test_histograms, fold + 1, folds, histograms_boolean
        )
...
def load_gmm(gaussian_mixtures_folder, descriptor_info, number_of_gaussians, norm_type):
    ...
    possible_files = [
        (int(f.split("_")[0].split(".")[0]), int(f.split("_")[3].replace("s", "")))
        for f in os.listdir(gaussian_mixtures_folder)
        if f.endswith(".npz")
        and int(f.split("_")[2].replace("N", "")) == number_of_gaussians
        and descriptor_info == f.split("_")[-2]
        and f.split("_")[-1] == str(norm_type + ".npz")
    ]
    ...
    try:
        sorted_indexes = np.argmax(
            np.asarray(possible_files)[:, 1]
        )  # agafem el model que sha fet amb mes samples
    except:
        return False
    models_with_max_samples = np.asarray(possible_files)[:, 0][
        sorted_indexes
    ]  # ordenem els sites amb els index

    file_name = [
        f
        for f in os.listdir(gaussian_mixtures_folder)
        if f.endswith(".npz")
        and int(f.split("_")[0].split(".")[0]) == models_with_max_samples
    ]
...
def save_gmm(
    ...
    number_of_gmm = np.sum(
        np.asarray([1 for f in os.listdir(folder_to_save) if f.endswith(".npz")])
    )
    gaussian_name = (
        str(int(number_of_gmm))
        + "_gmm_N"
        + str(number_gaussians)
        + "_"
        + str(number_samples)
        + "s_k"
        + str(fold + 1)
...
def print_results(results, result_names, round_):
    ...
    for result in results:
        ...
        print(
            "{:->35}".format(result_names[r_count] + ": ")
            + round_string
            + str([str("{:06.4f}".format(r)) for r in result])
        )
        r_count += 1


def save_results(
    google_drive_boolean,
    folder_to_save,
    class_type,
    regresor,
    folds,
    ...
    file_name = (
        ...
        + str(folds)
        + "_ngm"
        + str(number_of_gaussians)
        + "_"
        + str(total_samples)
        + "_"
        + str(samples_per_aqi_index)
        + ".txt"
    )
    ...
    myfile.write(
        "Keept portion of samples to do the GMM: "
        + str(gaussian_mixture_keep_portion)
        + "\n"
    )
    ...
    if google_drive_boolean:
        ...
        result_folder_id = save_file_google_drive_path(
            os.path.join(final_folder_to_save, file_name),
            "text/plain",
            "TFG/files/results/"
            + file_name.split("_")[0]
            + "_"
        ...
        for f in os.listdir(final_folder_to_save):
            if f.endswith(
                ".png"
            ):  # if the file is a png file, i.e. the samples histogram
                print("Saving histogram " + f + " into Google Drive")
                save_file_google_drive_id(
                    os.path.join(final_folder_to_save, f), "image/png", result_folder_id
                )
...
def compare_results(folder_with_results_txt, metric_used, regresors, fited_regressors):
    ...
    for file in os.listdir(folder_with_results_txt):
        if file.endswith(".txt"):  # if its a txt file
            ...
            for line_number in range(
                len(lines) - 1, 0, -1
            ):  # read it backwards (we will find the mean results first)
                ...
                if (
                    letters.strip() == "Mean results"
                ):  # if we have passed the Mean results line, from that everything are the results for each round
                    break
    ...
    max_index = [
        np.argmax(result_array[:, column]) for column in range(result_array.shape[1])
    ]

    comparison_file = open(
        os.path.join(folder_with_results_txt, "comparison_file.txt"), "w"
    )

    # parametter names
    [
        comparison_file.write(half_strip_formatter.format(p))
        for p in parameters_for_saving
    ]
    comparison_file.write("\n")
    # values list
    [
        comparison_file.write(
            half_strip_formatter.format(result_array[max_index[i], i])
        )
    ...
    # file name list
    [
        comparison_file.write(half_strip_formatter.format(result_file_names[i][0]))
        for i in max_index
    ]
    comparison_file.write("\n")
    ...
    for (
        ...
        best_reg_names
    ):  # per cada parametre regressor es el nom del regressor utilitzat
        ...
        for (
            ...
            used_regressor_names
        ):  # recorrer els que hem fet servir i buscar el que te el mateix nom que el maxim
            if (
                used_regressor == regressor.split("_")[-1]
            ):  # si es el maxim per aquest parametre pues retorna el numero
                print(used_regressor + "   " + regressor.split("_")[-1])
                break
            else:
                used_reg_count += 1
        max_regressor = fited_regressors[
            used_reg_count
        ]  # take the regressor corresponding to the best for this parametter
...
def visualize_residual_plots(
    plot_boolean, estimators, test_samples, test_labels, round
):
    count = 0
    for estimator in estimators:
        ...
        visualizer.score(
            test_samples, temp_test_labels
        )  # Evaluate the model on the test data
        if plot_boolean[1]:  # save them
            save_path = (
                plot_boolean[1]
                + "/residual plots of regressor "
                + str(estimator).split("(")[0]
                + " for parameter"
                + str(parameters_for_saving[count])
                + " and round "
                + str(round)
                + ".png"
            )
...
def drive_folder_id_from_path(service, path, parent_id, count):
    folder = path.split("/")[
        0
    ]  # for each folder in the drive path, starting for the root one
    if count == 0:  # first swarch we don't know the parent id
        results = (
            service.files()
            .list(  # get folders with name = ' '
    ...
    if path.replace((folder + "/"), "") != path:
        final_id = drive_folder_id_from_path(
            service,
            path.replace((folder + "/"), ""),
            results["files"][0]["id"],
            count + 1,
...
def save_file_google_drive_path(file_path, mimeType, google_drive_path):
    ...
    parent_folder_id = drive_folder_id_from_path(
        service, google_drive_path, 0, 0
    )  # parent_id==0 because we dont know it, wont be used in the first round
...
def copy_matlab_features_and_pollution_data_from_folder_to_folder(
    ...
    for sub_folder in os.listdir(from_folder):  # for each folder in the
        ...
        if not sub_folder.endswith(".DS_Store"):
            for file in os.listdir(os.path.join(from_folder, sub_folder)):
                if (
                    os.path.isdir(os.path.join(from_folder, sub_folder, file))
                    and file == "features"
                ):  # si es la carpeta de features
                    check_folder_and_create(
                        os.path.join(to_folder, sub_folder, file)
                    )  # crear carpeta
                    for feature in os.listdir(
                        os.path.join(from_folder, sub_folder, file)
                    ):  # copiar cada fitxer de la carpeta
                        copyfile(
                            os.path.join(from_folder, sub_folder, file, feature),
                            os.path.join(to_folder, sub_folder, file, feature),
                        )
...
if __name__ == "__main__":
    ...
    cameras_closer_than = pp.get_cameras_with_sites_closer_than(
        db_cursor,
        10,
        "O2_DISTANCE, SO2_DISTANCE, CO_DISTANCE, NO2_DISTANCE, PM25_DISTANCE, PM10_DISTANCE",
    )  # all camera paramters
