...
def main(args): 
    ...
    for reg in args.regresor:
        ...
        if reg == 'SVM':
            ...
            regresor = svm.SVR(kernel='rbf')#ElasticNet(random_state=0)#
        elif reg == 'SVM2':
            regresor = svm.SVR(kernel='linear')#ElasticNet(random_state=0)#
        elif reg == 'SVM3':
            regresor = svm.SVR(kernel='poly')#ElasticNet(random_state=0)#
        elif reg == 'SVM4':
            regresor = svm.SVR(kernel='sigmoid')#ElasticNet(random_state=0)#
        elif reg == 'SVM5':
            regresor = svm.SVR(kernel='precomputed')#ElasticNet(random_state=0)#
        elif reg == 'LASS':
            regresor=linear_model.Lasso(alpha=0.1)
        elif reg == 'LASS1':
            regresor=linear_model.Lasso(alpha=0.2)
        elif reg == 'LASS2':
            regresor=linear_model.Lasso(alpha=0.3)
        elif reg == 'LASS3':
            regresor=linear_model.Lasso(alpha=0.5)
        elif reg == 'LASS4':
            regresor=linear_model.Lasso(alpha=0.6)
        elif reg == 'LASS5':
            regresor=linear_model.Lasso(alpha=0.7)
        elif reg == 'LASS6':
            regresor=linear_model.Lasso(alpha=0.8)
        elif reg == 'LASS7':
            regresor=linear_model.Lasso(alpha=0.9)
        elif reg == 'LASS8':
    ...
    clases_to_get=[int(i) for i in os.listdir(folder) if i != '.DS_Store']#[42,129]    
    # if args.not_all_data:
    clases_to_get=[32,37,42,47,52,57,62,67,72,77,82,87,92]
...
if __name__ == "__main__":
    ...
    default_regresor=['SVM']
    default_patch_sizes=[12]
    default_descriptors=['sift']#'color', 'centrist'
