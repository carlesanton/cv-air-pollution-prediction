...
class camara:
    ...
    def __str__(self):
        return ('ID: ' + str(self.id) + '    LAT: '+ str(self.lat) + '    LON: '+ str(self.long) + '    ' + self.full_address)

class monitor:
    def __init__(self, id, lat, long, state, county, city, full_address,parameter_code ,scale_definition):
        self.id = id
        self.lat = lat
        self.long = long
        self.city = city
    ...
    def add_to_database(self, cursor: sqlite3.Cursor):
        ...
        #mira si no hi ha cap objete amb aquella ID, si no hi ha el posa
        if(len(objectes)==0):
            cursor.execute("insert into monitors values (?,?,?,?,?,?,?,?,?,?)", (self.id,monitor_id,self.parameter_code , self.lat, self.long, self.state, self.county, self.city, self.full_address, self.scale_definition))
        '''
        else:
...
class site:
    def __init__(self, id, lat, lon):
        ...
        self.long = lon
    def __str__(self):
        return ('site ID: ' + str(self.id) + '    LAT: '+ str(self.lat) + '    LON: '+ str(self.long))
    def __repr__(self):
        return str(self.id)
...
def get_cameras(cursor: sqlite3.Cursor, print_bool=False):
    ...
    with db_connection:
        ...
        for cam in totes_cameres:
            ccc=camara( cam[0], cam[1], cam[2], cam[9], cam[8], cam[7], cam[6],cam[5],cam[4], cam[3], cam[10])
            cams.append(ccc)
            if print_bool:
                print(ccc)
            count+=1
...
def get_cameras_condition(condition, cursor: sqlite3.Cursor):
    ...
    count=0
    for cam in totes_cameres:
        cams.append(camara( cam[0], cam[1], cam[2], cam[9], cam[8], cam[7], cam[6],cam[5],cam[4], cam[3], cam[10]))
        count+=1
    return cams
def iterate_csv_monitors(path):
    ...
    for row in reader:
        if count !=0:
            ...
            if len(row[7])!=0:
                lon=float(row[7])
            final_id=row[0]+'-'+row[1]+'-'+row[2].replace('"','')+'-'+row[3]
            suport_monitor=monitor(final_id,lat,lon,row[26],row[27],row[28],row[25],row[3],row[21])
...
def iterate_csv_sites(path, cursor: sqlite3.Cursor):
    ...
    for row in reader:
        if count !=0:
            ...
            cursor.execute("insert into sites values (?,?,?,?,?,?,?,?)", (final_id,lat,lon , row[22],row[23],row[24],row[10],row[14]))
...
def get_monitors_condition(condition, cursor: sqlite3.Cursor):
    ...
    for mon in tots_monitors:     
        monit.append(monitor( mon[1], mon[3], mon[4], mon[5], mon[6], mon[7], mon[8],mon[2],mon[9]))
        #print(count)
        count+=1
    return monit
...
def create_camera_site_table(cursor:sqlite3.Cursor):
    ...
    db_cursor.execute('''CREATE TABLE IF NOT EXISTS camera_sites (camera_id INTEGER PRIMARY KEY, whole_site_id CHARACTER,distance_km INTEGER)''')
    ...
    for c in cams:
        ...
        while len(get_sites)==0:
            ...
            get_condition="where closing_date = '' AND lat < "+ str(bbox[0]) +" AND lat > " +str(bbox[1])+" AND long < " +str(bbox[2])+" AND long > " +str(bbox[3])+ " ORDER BY lat DESC"
        ...
        cursor.execute("insert into camera_sites values (?,?,?)", (c.id,sorted_sites[0].id,math.floor(distances[sorted_indexes[0]])))
        print("Camera: "+ str(c.id) + " paired with site: "+ str(sorted_sites[0].id) +" at distance " + str(distances[sorted_indexes[0]]))
...
def get_cameras_with_sites(cursor:sqlite3.Cursor):
    ...
    for r in totes:
        ...
        #print([r[0],r[1],r[2]])
    return relations
def get_usa_monitor_data(format, param, bdate, edate, state, county, site,destiny_path):
    ...
    raw_parameters = {'user' : API_USER,'pw' : API_PASSWORD,'format':format,'param' : param, 'bdate' : bdate, 'edate' : edate, 'state' : state, 'county' : county, 'site' : site};  
    with requests.Session() as s:
        ...
        count=0
        for row in cr:        
            if not cr.line_num == 1 and row[0] != 'END OF FILE':#si no es la primera ni esta al final
                data=str(row[12]).split('-')#agafar la data
                anymes=data[0]+data[1]#posarho en YYYYMM format
                file_name=camera_id_string+'_'+anymes+'_'+str(param)+'.txt'#idcam_anymes_param
                final_file_path=destiny_path+"/"+file_name#src/idcam/idcam_anymes_param
                if not os.path.exists(final_file_path): #si no existeix
                    myfile = open(final_file_path,"w")
                    myfile.write(str(row[7])+'    '+str(row[9])+' ('+str(row[17])+')'+'\n') #linea de info
                    myfile.write(str(data[2]+'/'+row[13])+'    '+str(row[16])+'\n')
                elif os.path.exists(final_file_path): #si existeix
                    myfile = open(final_file_path,"a")
                    myfile.write(str(data[2]+'/'+row[13])+'    '+str(row[16])+'\n')
                myfile.close()
        print(' '+str(cr.line_num-2)+' lines read')    
def get_site_data(site_id, data_inici, data_final,cursor: sqlite3.Cursor,destiny_path):
    check_folder_and_create(destiny_path,1)
    monitors_lloc=get_monitors_condition("where monitor_id = '" + site_id + "'", cursor)
    cc=1
    for m in monitors_lloc:
        site_number = m.id.split("-")[2]
        print ('Downloading data from site '+str(site_id)+': monitor ' + str(cc)+'/'+str(len(monitors_lloc))+', parameter: '+str(m.parameter_code),end="")
        sys.stdout.flush()
        get_usa_monitor_data('DMCSV',m.parameter_code,data_inici,data_final,m.id.split("-")[0],m.id.split("-")[1],site_number,destiny_path)
        cc+=1
def get_sites_list_data(relations_lists,data_inici,data_final,cursor: sqlite3.Cursor):
	for s in relations_lists:
		get_site_data(s[1], data_inici,data_final,cursor,str(src_path+'/data/'+str(s[0]).zfill(8)))
...
def parse_aqi(cursor: sqlite3.Cursor,path):
    ...
    for row in reader:
        ...
        if reader.line_num !=1:
            ...
            cursor.execute("insert into aqi_index values (?,?,?,?)", ((row[6]+'/'+row[2]),row[3],row[5],row[6]))
            if count%100==0:
                print(count)
        count+=1
...
def parse_rh_dewpoint(cursor: sqlite3.Cursor,path):
    ...
    for row in reader:
        ...
        if reader.line_num !=1:
            ...
            cursor.execute("insert into wind_speed values (?,?,?,?)", ((row[6]+'/'+row[2]),row[3],row[5],row[6]))
            if count%100==0:
                print(count)
        count+=1
...
def get_drive_folders_in_id(service,folder_id):
    query="mimeType = 'application/vnd.google-apps.folder'  and '" + folder_id+"' " + "in parents"
...
def get_drive_folders_in_id_with_name(service,folder_id,target_folder_name):
    query="mimeType = 'application/vnd.google-apps.folder'  and '" + folder_id+"' " + "in parents and name = " + "'" +target_folder_name+"'"
    try:
        results = service.files().list(
            q=query,
            spaces='drive',
...
def get_cam_year_files_list(service,camera_id,year,images_folder_id):
    desired_year_folder = get_drive_folders_in_id_with_name(service,images_folder_id,str(year))
    cam_id=re.findall('..', str(camera_id))
    zip_files_list=[]
    if desired_year_folder:
        
        half_id_folder = get_drive_folders_in_id_with_name(service,desired_year_folder[0]['id'],cam_id[-1])
        if half_id_folder:
            next_half = get_drive_folders_in_id_with_name(service,half_id_folder[0]['id'],str(cam_id[-2]+cam_id[-1]))
            if next_half:
                final_folder_id = get_drive_folders_in_id_with_name(service,next_half[0]['id'],camera_id)
                if final_folder_id:
                    zip_files_list=get_drive_files_in_id(service,final_folder_id[0]['id'])
                else:
...
def get_all_images_in_list(camera_list,year):
    ...
    image_folder = get_drive_folders_in_id_with_name(service,amos_folder[0]['id'],'images')
    for camera in camera_list:
        #referesh creds to avoid timeout
        creds = get_google_drive_credentials(creds, creds_name)
        service = build('drive', 'v3', credentials=creds)
        c_id=camera.zfill(8)
        zip_files_list=get_cam_year_files_list(service,c_id,year,image_folder[0]['id'])
        if zip_files_list:
        ...
        if zip_files_list:
            ...
            try:
                ...
                if unzip_file_to(file_path,final_folder,1):
                    ...
                    fotos_a_les_15=[f for f in imatges_a_la_carpeta if(re.findall('..', str(f.split('_')[1]))[0]==str(15))] # if(re.findall('..', str(f.split('_')[1]))[0]==str(15))
                    ...
                    copy_image_to_folder(final_folder+'/'+imatge_sample,sample_folder,(year+'_'+c_id))
...
def check_night_image(image_path,area_threshold,pixel_threshold):
    ...
    final_uint8_image=((np.dstack((grey_image,grey_image,grey_image))) .astype(np.uint8))
    grey_pixels_number=np.sum((img[:,:,0]==img[:,:,1]) & (img[:,:,1]==img[:,:,2]))
...
def check_night_folder(folder_to_check,area_th,pixel_th):
    ...
    for i in image_list:
        if i.endswith(".jpg"):#si es imatge
            ...
            if check_night_image(im_path,area_th,pixel_th):#mirem si es imatge en gris
...
def copy_list_from_to(names_list, original_path,destiny_path):
    ...
    for f in files_in_folder:
        if f.endswith('.jpg'):
            ...
            if [True for n in names_list if '{:0>8}'.format(str(n)) == cam_id]:#si la camara esta a samples_images (q hauria de)
                count+=1
                check_folder_and_create(destiny_path,1)
...
def make_image_patches(patch_size,original_folder_path,image_name,destiny_folder_path,*args):
    ...
    for x in range(0,tamany[0],x_step):
        for y in range(0,tamany[1],y_step): 
            ...
            if len(patches_to_do)==0:
                must_do_patch=True
            elif patches_to_do[patches_done_count]==count:#si el patch que anem a fer es el següent que hem de fer
                must_do_patch=True
            if must_do_patch:
                if patches_done_count < len(patches_to_do)-1:
                    patches_done_count+=1
                #print('done')
                if not os.path.exists(destiny_folder_path+'/'+image_name.replace('.'+format,'')+'_patch'+str(count)+'.'+format):
                    result.save(destiny_folder_path+'/'+image_name.replace('.'+format,'')+'_patch'+str(count)+'.'+format)
            must_do_patch=False    
            count+=1
...
def save_patches_to_do(folder_path):
    for f in os.listdir(folder_path):
        if f.endswith('.jpg'):
            ...
            print('Saving patches names of ' + cam_id + ' to ' + folder_path+'/'+file_name)
            if not os.path.exists(folder_path+'/'+file_name):
                myfile = open(folder_path+'/'+file_name,"w")
            else:
                myfile = open(folder_path+'/'+file_name,"a")
...
if __name__ == '__main__':
    ...
    print("USA camera % of total relation = " + '{:02.2f}'.format(100*float(len(cams)/len(totes_cameras))) )
